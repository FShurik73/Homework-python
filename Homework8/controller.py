...
def input_parrents(command: int):
    # Обработка запросов родителей
    match command:
        case 1:
            name = view.get_name()
            if model.check_name(model.get_journal(),name):
                view.show_pupil_lesson(model.get_journal(), name)
            else:view.shmessage(TEXT_INFO)
        case 2:
...
def start():
    ...
    if access != 1:
        ...
        while True:
            ...
            input_parrents(command)

def control_knowledge():
    # Контроль знаний ученика
    lesson = view.get_lesson()
    if model.check_lesson(model.get_journal(), lesson):
        view.show_message('Отвечать будет...')
        name = view.get_name()
        if model.check_name(model.get_journal(), name):
            mark = view.control()
