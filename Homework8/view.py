...
def choice_class() -> str:
    ...
    while True:
        try:
            class_num = input('\n Введите класс: ')
            if class_num.lower() not in ['5а', '5б']:
                print('Доступны класс 5А или класс 5Б')
            elif class_num.lower() in ['5а', '5б']:
...
def command_parents() -> int:
    ...
    # Получение команды от родителей :return: номер команды.
...
def get_name() -> str:
    # Запрос имени ученика :return: Фамилия и имя ученика с заглавных букв.
...
def control() -> int:
    # Контроль знаний ученика.:return: оценка.
    while True:
        try:
